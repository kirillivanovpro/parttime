...
function Button({ children, onClick, variant = "solid", className = "", type = "button", disabled = false }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled} className={`inline-flex items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-black transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 ${styles[variant]} ${className}`}>
      {children}
    </button>
  );
}
...
function Avatar({ profile, size = "md" }) {
  ...
  return <div className={`${sizes[size]} flex shrink-0 items-center justify-center rounded-full bg-indigo-100 font-black text-indigo-700`}>{profile?.full_name?.[0]?.toUpperCase() || "U"}</div>;
}

function Logo() {
  return (
    <div className="flex items-center gap-2 font-black tracking-tight text-slate-950">
      <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-slate-950 text-white shadow-lg shadow-slate-300">P</div>
      <span className="text-2xl">
        Part<motion.span animate={{ opacity: [1, 0.15, 1] }} transition={{ duration: 1.2, repeat: Infinity }} className="text-indigo-600">:</motion.span>time
      </span>
    </div>
  );
}
...
function Header({ screen, setScreen, session, profile, onLogout }) {
  const nav = [["home", "Главная"], ["feed", "Задания"], ["create", "Создать"], ["chats", "Чаты"], ["profile", "Профиль"]];
  return (
    <header className="sticky top-0 z-40 border-b border-slate-100 bg-white/85 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-2 md:flex">
          {nav.map(([id, label]) => (
            <button key={id} onClick={() => setScreen(id)} className={`rounded-full px-4 py-2 text-sm font-semibold transition ${screen === id ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100 hover:text-slate-950"}`}>{label}</button>
          ))}
        </nav>
        <div className="flex items-center gap-2">
          {session ? (
            <>
              <button onClick={() => setScreen("profile")} className="hidden items-center gap-2 rounded-full border border-slate-200 px-3 py-2 text-sm font-black text-slate-700 sm:flex">
                <Avatar profile={profile} size="sm" /> {profile?.full_name || session.user.email}
              </button>
              <Button onClick={onLogout} variant="outline" className="rounded-full">Выйти</Button>
            </>
          ) : (
            <Button onClick={() => setScreen("auth")} variant="outline" className="rounded-full">Войти</Button>
          )}
        </div>
      </div>
    </header>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 text-white">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-20 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-28">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
            ...
            <h1 className="mt-6 max-w-3xl text-5xl font-black leading-tight tracking-tight sm:text-6xl">Найди помощь или подработку рядом</h1>
            <p className="mt-6 max-w-xl text-lg leading-8 text-slate-300">Part:time соединяет людей, у которых есть задачи, с теми, у кого есть свободное время и желание заработать.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Button onClick={() => setScreen("create")} variant="green" className="h-14 px-7 text-base">Создать задание</Button>
              <Button onClick={() => setScreen("feed")} className="h-14 bg-white/10 px-7 text-base text-white hover:bg-white/20">Найти подработку</Button>
            </div>
          </motion.div>
            ...
            <div className="rounded-[1.5rem] bg-slate-50 p-5 text-slate-950">
              ...
              <div className="mt-4 grid gap-3">
                {["Пользователи и профили", "Фото аватаров", "Задания", "Отклики", "Сообщения", "Отзывы и рейтинг"].map((item) => (
                  <div key={item} className="rounded-2xl bg-white p-4 font-bold shadow-sm">✓ {item}</div>
                ))}
              </div>
            </div>
          </div>
  ...
  async function submit() {
    ...
    try {
      ...
      if (mode === "register") {
        ...
        if (!data.session) {
          setMessage("Аккаунт создан. Если включено подтверждение email, проверьте почту. Для быстрого MVP можно отключить Confirm email в Supabase Auth settings.");
          return;
        }
    ...
    <main className="min-h-screen bg-slate-50">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1fr_460px] lg:px-8">
        <section className="rounded-[2rem] bg-slate-950 p-8 text-white">
          ...
          <p className="mt-4 leading-8 text-slate-300">Профиль, рейтинг, отзывы и история взаимодействий создают доверие между заказчиком и исполнителем.</p>
        </section>

        <section className="rounded-[2rem] bg-white p-6 shadow-sm sm:p-8">
          <div className="flex rounded-2xl bg-slate-100 p-1">
            <button onClick={() => setMode("login")} className={`flex-1 rounded-xl px-4 py-3 text-sm font-black ${mode === "login" ? "bg-white shadow-sm" : "text-slate-500"}`}>Вход</button>
            <button onClick={() => setMode("register")} className={`flex-1 rounded-xl px-4 py-3 text-sm font-black ${mode === "register" ? "bg-white shadow-sm" : "text-slate-500"}`}>Регистрация</button>
          </div>
          ...
          <div className="mt-6 grid gap-4">
            {mode === "register" && (
              <>
                <input value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} placeholder="Имя" className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500" />
                <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="Телефон" className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500" />
                <input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} placeholder="Город" className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500" />
                <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500">
            ...
            <input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="Email" className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500" />
            <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="Пароль" className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500" />

            {message && <div className="rounded-2xl bg-indigo-50 p-4 text-sm font-bold leading-6 text-indigo-800">{message}</div>}

            <Button onClick={submit} disabled={loading} className="h-14 text-base">{loading ? "Подождите..." : mode === "login" ? "Войти" : "Зарегистрироваться"}</Button>
          </div>
        </section>
      </div>
    </main>
    ...
    <main className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 rounded-[1.5rem] bg-white p-3 shadow-sm">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Поиск: доставка, уборка, дизайн..." className="w-full rounded-2xl bg-slate-50 px-4 py-3 outline-none" />
        </div>
            ...
            return (
              <div key={task.id} className="rounded-[1.5rem] border border-slate-100 bg-white p-5 shadow-sm">
                ...
                <div className="mt-5 flex gap-3">
                  <Button onClick={() => { setSelectedTask(task); setScreen("task"); }} className="flex-1">Подробнее</Button>
                </div>
              </div>
            );
          })}
      ...
      <main className="min-h-screen bg-slate-50">
        <div className="mx-auto max-w-3xl px-4 py-12">
          <div className="rounded-[2rem] bg-white p-8 text-center shadow-sm">
            ...
            <Button onClick={() => setScreen("auth")} className="mt-6">Войти / зарегистрироваться</Button>
          </div>
        </div>
      </main>
    );
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        <div className="rounded-[2rem] bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="mt-8 grid gap-5">
            <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Что нужно сделать?" className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500" />
            <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500">
              {categories.map((c) => <option key={c.label}>{c.label}</option>)}
            </select>
            <div className="grid gap-5 sm:grid-cols-2">
              <input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} placeholder="Город" className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500" />
              <input type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="Бюджет, €" className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500" />
            </div>
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={5} placeholder="Детали" className="rounded-2xl border border-slate-200 px-4 py-4 outline-none focus:border-indigo-500" />
            {message && <div className="rounded-2xl bg-red-50 p-4 text-sm font-bold text-red-700">{message}</div>}
            <Button onClick={submit} disabled={loading} className="h-14 text-base">{loading ? "Публикуем..." : "Опубликовать задание"}</Button>
          </div>
        </div>
      </div>
    </main>
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_360px] lg:px-8">
        <section className="rounded-[2rem] bg-white p-6 shadow-sm sm:p-8">
          <button onClick={() => setScreen("feed")} className="text-sm font-bold text-indigo-600">← Назад к заданиям</button>
          <div className="mt-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
            <div>
              ...
              <div className="mt-4 text-slate-500">{icons.map} {task.city}</div>
            </div>
          ...
          <div className="mt-8 border-t border-slate-100 pt-8">
            ...
            <p className="mt-3 max-w-3xl leading-8 text-slate-600">{task.description}</p>
          </div>
        </section>

        <aside className="rounded-[2rem] bg-white p-6 shadow-sm">
          <h3 className="text-lg font-black">Откликнуться</h3>
          {!session && <div className="mt-4 rounded-2xl bg-indigo-50 p-4 text-sm font-bold leading-6 text-indigo-800">Чтобы откликнуться, нужно войти.</div>}
          <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={4} className="mt-4 w-full rounded-2xl border border-slate-200 p-4 outline-none focus:border-indigo-500" placeholder="Напишите коротко, когда можете выполнить задачу." />
          <Button onClick={apply} className="mt-3 h-12 w-full">{icons.send} Отправить отклик</Button>
        ...
        <section className="flex min-h-[620px] flex-col rounded-[2rem] bg-white shadow-sm">
          ...
          <div className="border-t border-slate-100 p-4">
            <div className="flex gap-2 rounded-3xl bg-slate-50 p-2">
              <input disabled={!session} value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") sendMessage(); }} className="flex-1 bg-transparent px-3 outline-none disabled:opacity-50" placeholder={session ? "Написать сообщение..." : "Войдите, чтобы писать"} />
              <Button onClick={sendMessage} disabled={!session}>{icons.send}</Button>
            </div>
          </div>
        </section>
      </div>
...
function Profile({ session, profile, setProfile, reviews, setScreen }) {
      ...
      <main className="min-h-screen bg-slate-50">
        <div className="mx-auto max-w-3xl px-4 py-12">
          <div className="rounded-[2rem] bg-white p-8 text-center shadow-sm">
            ...
            <Button onClick={() => setScreen("auth")} className="mt-6">Войти</Button>
          </div>
        </div>
      </main>
    );
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 text-slate-600">Профиль, фото, рейтинг и отзывы сохраняются в Supabase.</p>
          </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[380px_1fr]">
          ...
          <section className="rounded-[2rem] bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <input value={draft?.full_name || ""} onChange={(e) => setDraft({ ...draft, full_name: e.target.value })} placeholder="Имя" className="rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-indigo-500" />
              <input value={draft?.phone || ""} onChange={(e) => setDraft({ ...draft, phone: e.target.value })} placeholder="Телефон" className="rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-indigo-500" />
              <input value={draft?.city || ""} onChange={(e) => setDraft({ ...draft, city: e.target.value })} placeholder="Город" className="rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-indigo-500" />
              <select value={draft?.role || "both"} onChange={(e) => setDraft({ ...draft, role: e.target.value })} className="rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-indigo-500">
                <option value="both">Заказчик и исполнитель</option>
                <option value="customer">Только заказчик</option>
                <option value="performer">Только исполнитель</option>
              </select>
              <textarea value={draft?.bio || ""} onChange={(e) => setDraft({ ...draft, bio: e.target.value })} placeholder="О себе" rows={4} className="rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-indigo-500 sm:col-span-2" />
            </div>

            <div className="mt-5 flex flex-wrap gap-3">
              <Button onClick={saveProfile}>Сохранить профиль</Button>
              <Button onClick={() => setDraft({ ...draft, verified: true })} variant="outline">Демо: пройти проверку</Button>
              <Button onClick={addDemoReview} variant="outline">Демо: добавить отзыв</Button>
            </div>
        ...
        <section className="mt-6 rounded-[2rem] bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-3">
            {myReviews.length === 0 ? <div className="rounded-3xl bg-slate-50 p-5 text-slate-500">Пока нет отзывов.</div> : myReviews.map((r) => (
...
function BottomNav({ screen, setScreen }) {
  const items = [["feed", icons.briefcase, "Задания"], ["create", icons.plus, "Создать"], ["chats", icons.message, "Чаты"], ["profile", icons.user, "Профиль"]];
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 px-2 py-2 backdrop-blur md:hidden">
      <div className="grid grid-cols-4 gap-1">
        {items.map(([id, icon, label]) => (
          <button key={id} onClick={() => setScreen(id)} className={`flex flex-col items-center justify-center rounded-2xl px-2 py-2 text-xs font-bold ${screen === id ? "bg-indigo-50 text-indigo-700" : "text-slate-500"}`}>
...
export default function ParttimeApp() {
  ...
  async function loadEverything(activeSession = session) {
    ...
    const [{ data: taskRows }, { data: profileRows }, { data: messageRows }, { data: reviewRows }] = await Promise.all([
    ...
    const byId = {};
    (profileRows || []).forEach((p) => { byId[p.id] = p; });
    setProfiles(byId);
  ...
  return (
    <div className="min-h-screen bg-white pb-20 font-sans text-slate-950 md:pb-0">
      ...
      {screen === "profile" && <Profile session={session} profile={profile} setProfile={setProfile} reviews={reviews} setScreen={setScreen} />}
